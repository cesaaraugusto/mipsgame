...
.macro ubicar(%x, %y)
    ...
    mfhi  $s0                # $s0 ?? residuo
    bltz  $s0, adjust_x      # Si el residuo es negativo, aj�stalo
    j     compute_y
adjust_x:
...
.text
   ...
   main:
    lw   $t0, 0xFFFF0004         # Carga en $t0 la tecla presionada desde la direcci�n 0xFFFF0004
    ...
    lw   $t2, j_sapo              # Carga en $t2 la posici�n actual del sapo (variable j_sapo)
    ...
    sw   $zero, 0xFFFF0004         # Limpia la direcci�n 0xFFFF0004 (resetea el valor de la tecla)
    b    bucle             # Salta al �bucle?? para las actualizaciones y dibujo

sapo_sube:
    sub  $t2, $t2, sapo_velocidad  # Resta la velocidad a la posici�n actual para mover el sapo a la izquierda
    sw   $t2, j_sapo              # Actualiza la posici�n del sapo en la variable j_sapo
    b    bucle             # Salta a �bucle??

sapo_baja:
    add  $t2, $t2, sapo_velocidad  # Suma la velocidad a la posici�n actual para mover el sapo a la derecha
    sw   $t2, j_sapo              # Actualiza la posici�n en j_sapo
    b    bucle             # Salta a �bucle??

lengua:
    sw   $t0, tecla_lengua        # Guarda en tecla_lengua el c�digo de la tecla presionada (activa la lengua)
    add  $t2, $t2, 29             # Ajusta la posici�n de la lengua sumando 27 a la posici�n actual del sapo
    sw   $t2, j_lengua            # Guarda la posici�n resultante en j_lengua (coordenada X de la lengua)
    li   $t2, i_sapo_posicion     # Carga en $t2 el valor base de la posici�n del sapo (i_sapo_posicion)
    add  $t2, $t2, espacio_sapo_ancho  # Suma a ese valor el ancho del sapo (espacio_sapo_ancho)
    sw   $t2, i_lengua_inicial    # Define i_lengua_inicial con el valor calculado (l�mite izquierdo de la l�nea)
...
mosca5_lista:
    ...
    lw   $t0, tecla_lengua        # Carga el valor de tecla_lengua para verificar si la lengua est� activa
    ...
    ubicar($t0, $t1)              # Convierte las coordenadas l�gicas (i_lengua_inicial, j_lengua) a reales, dejando $s0 y $s1
    ...
    add  $t0, $t0, $s0           # Suma la coordenada x real para obtener el �ndice lineal
    mul  $t0, $t0, 4             # Convierte el �ndice lineal a byte-offset (4 bytes/p�xel)
    add  $s0, $t0, DIRECCION_DISPLAY  # Suma la direcci�n base del framebuffer y obtiene la direcci�n real del p�xel
    lw   $t0, ($s0)             # Carga el color del p�xel ubicado en la direcci�n calculada
    lw   $t6, mosca1_exists
    lw   $t7, mosca2_exists
    lw   $t8, mosca3_exists
    add $t6, $t6, $t7
...
lengua_sin_darle_al_blanco:
    lw   $t0, 0xFFFF0004           # Recarga el valor de la tecla desde la direcci?n de entrada
    beq  $t0, 0x61, limpiar_lengua  # Si la tecla es 0x61, salta a limpiar_lengua (desactiva la lengua)
    
    lw   $t3, i_lengua_final       # Carga la posici?n X actual del extremo de la lengua (i_lengua_final)
    li   $t8, 489                  # L?mite derecho de la lengua
    bge  $t3, $t8, limpiar_lengua  # Si i_lengua_final >= 489, salta a limpiar_lengua (no se dibuja m?s)
    ...
    # Calcula la direcci?n del p?xel en el framebuffer:
    ...
    mul  $t0, $t0, 4               # Multiplica por 4 para obtener la posici?n en bytes
    add  $s0, $t0, DIRECCION_DISPLAY # Suma la base del framebuffer para obtener la direcci?n final

    # Comprobaci?n del color actual del p?xel:
    lw   $t4, ($s0)               # Carga el color actual del p?xel en $t4
    li   $t5, -16711420            # Carga el color prohibido (#141510) en $t5
    beq  $t4, $t5, mosca_comida   # Si el p?xel ya es de color #141510, salta a limpiar_lengua

    # --- Bloque agregado para imprimir el color actual ---
    la   $a0, msg_color           # Carga la direcci?n del mensaje "Color actual del pixel: "
    ...
    sw   $t2, ($s0)               # Escribe el color en el p?xel (dibuja la primera fila de la lengua)

    lw   $t1, i_lengua_final      # Recarga el valor actual de i_lengua_final
    li   $t8, 489                 # Carga nuevamente el l?mite derecho (489)
...
mosca_comida:
    lw $t3, i_lengua_final
    lw $t4, i_mosca1
    beq $t3, $t4, mosca1_comida
mosca1_comida:
    lw   $t0, i_mosca1
    lw   $t1, j_mosca1
    pintar(espacio_efecto_bueno, espacio_efecto_ancho,espacio_efecto_altura, $t0, $t1)
    lw   $t0, i_mosca1
    lw   $t1, j_mosca1
    pintar(espacio_mosca_mala_abajo, espacio_mosca_ancho,espacio_mosca_altura, $t0, $t1)
    sw   $zero, mosca1_exists
    b    limpiar_lengua

limpiar_lengua:
    sw   $zero, tecla_lengua           # Resetea la bandera de activaci�n de la lengua
...
clear_loop:
    ...
    lw   $t1, i_lengua_inicial         # Carga el l�mite izquierdo de la lengua
    ...
    mul  $t3, $t3, ANCHO_FB           # Multiplica y_real por el ancho del framebuffer (calcula offset vertical en p�xeles)
    add  $t3, $t3, $s0                # Suma la coordenada X real ($s0) para formar el �ndice lineal del p�xel
    li   $t4, 4                      # Carga el valor 4, que representa 4 bytes por p�xel
    mul  $t3, $t3, $t4               # Convierte el �ndice lineal a un offset en bytes
    add  $t3, $t3, DIRECCION_DISPLAY  # A�ade la direcci�n base del framebuffer para obtener la direcci�n f�sica del p�xel
    li   $t5, 0x84945C             # Carga el color de fondo (0x00000000) para borrar el p�xel
    sw   $t5, ($t3)                 # Escribe el color de fondo en el p�xel de la primera fila

    lw   $t6, i_lengua_final         # Carga el valor actual de i_lengua_final
    addi $t6, $t6, -1               # Decrementa i_lengua_final en 1 (retracci�n de la lengua)
    sw   $t6, i_lengua_final         # Actualiza la variable i_lengua_final en memoria

    j    clear_loop                 # Repite el ciclo para borrar el siguiente p�xel de la lengua

clear_done:
    j    bucle                     # Una vez que se hayan borrado todos los p�xeles necesarios, salta al ciclo principal (bucle)


pintar_sapo:
    lw   $t0, 0xFFFF0004                     # Carga la tecla presionada desde 0xFFFF0004
    beqz $t0, final_sapo                     # Si no se presion� ninguna tecla, salta a final_sapo
    lw   $t0, j_sapo                         # Carga la posici�n Y actual del sapo (j_sapo)
    pintar(espacio_sapo, espacio_sapo_ancho, espacio_sapo_altura, i_sapo_posicion, $t0)
    j    final_sapo                         # Salta a final_sapo despu�s de pintar
...
ganar:
    pintar(espacio_final_bueno,espacio_final_ancho,espacio_final_altura,0,0)
    b    salida
...
salida:
    li   $v0, 10             # Carga el c�digo 10 del syscall (exit)
